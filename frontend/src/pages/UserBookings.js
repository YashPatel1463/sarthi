...
function UserBookings() {
  ...
  const filteredBookings = localStorage.getItem('admin') 
  ? bookings  // Show all bookings if 'adminlog' is in the pathname
  : bookings.filter(o => o.user === user._id);  // Show only user's bookings

  return (
    <DefaultLayout>
        {loading && (<Spinner />)}
      <h3 className="text-center mt-2">{localStorage.getItem('admin') ? 'All Bookings' : 'My Bookings'}</h3>
        ...
        {filteredBookings.map((booking) => {
             return <Row gutter={16} className="bs1 mt-3 text-left">
                <Col lg={6} sm={24}>
                    ...
                    <p> Booked by : <b> {booking.user} </b></p>
